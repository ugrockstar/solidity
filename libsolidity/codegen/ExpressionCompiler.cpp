...
bool ExpressionCompiler::visit(Assignment const& _assignment)
{
	...
	if (op != Token::Assign)
		cleanupNeeded = m_context.checkedArithmetic() || cleanupNeededForOp(leftType.category(), binOp);
...
bool ExpressionCompiler::visit(UnaryOperation const& _unaryOperation)
{
	...
	case Token::Dec: // -- (pre- or postfix)
		...
		{
			if (m_context.checkedArithmetic())
...
bool ExpressionCompiler::visit(BinaryOperation const& _binaryOperation)
{
	...
	{
		bool cleanupNeeded = m_context.checkedArithmetic() || cleanupNeededForOp(commonType->category(), c_op);
